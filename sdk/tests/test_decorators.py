...
from data_lineage_hub_sdk.decorators import lineage_track, telemetry_track
...
def mock_lineage_client():
    ...
    with patch('data_lineage_hub_sdk.decorators.LineageHubClient') as mock:
...
def mock_tracer():
    ...
    with patch('data_lineage_hub_sdk.decorators.trace.get_tracer') as mock:
        tracer = mock.return_value
        span = MagicMock()
        tracer.start_as_current_span.return_value.__enter__ = MagicMock(return_value=span)
        tracer.start_as_current_span.return_value.__exit__ = MagicMock(return_value=None)
        yield tracer, span
...
class TestLineageTrackDecorator:
    ...
    async def test_decorator_handles_exceptions(self, mock_lineage_client):
        ...
        assert "Something went wrong" in fail_event["run"]["facets"]["error_info"]["error_message"]
